...
export class UserService {
  ...
  emitCreateUser = new EventEmitter<string>();

  // static staticEmitCreateUser = new EventEmitter<string>();
  ...
  constructor(private http: Http,
      ...
      console.log('Service Instanciado');
  }
  ...
  getUser(id: number): Observable<User> {
    ...
    return this.http.get(`${this.usersUrl}/${id}`, { headers })
      ...
      .map(this.toUser)
      .do(res => {
        this.logService.consoleLog(`Dados do Usuario ${res.name}`);
      })
  ...
  createUser2(user: User): Observable<User[]> {
    return this.http.post(this.usersUrl, user)
      .map(res => res.json())
      .do(user => {
        this.emitCreateUser.emit('User was created!');
        // UserService.staticEmitCreateUser.emit('User was created');
      });
  }
